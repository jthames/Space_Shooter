...
//Need to figure out how to do double buffering.. 
		...
		Runnable r13 = new Runnable()
		{
			...
			{   
				...
				Main.Game_Screen.setDoubleBuffered(true);				
			    	  ...
			    	  level =  new File(System.getProperty("user.dir")+"/levels/level1.txt"); 
				...
				while(!levelFinished && Ship_Alive)
				{  
					if(Main.Game_Screen.all_spawned == 0)
					{	
				      int num = 0;
				      int [] nums = new int[6];
				      boolean done = false;
				      ...
				      sc.close();
					Main.Game_Screen.all_spawned = 1;
				  }
		...
		Runnable r15 = new Runnable()
		{
			...
			{     
				...
				while((Ship_Alive && !levelFinished && set == 0) || (!Ship_Alive && !levelFinished && set == 0))
				{
				    ...
				    else if(enemies.enemies.size() == 0 && Enemy_Asteroids.size() == 0 && Main.Game_Screen.all_spawned == 1)
				    {
				    	thr1.stop();
			    	    thr2.stop();
			    		thr3.stop();
			    		...
			    		Main.Game_Screen.setVisible(false);
			 			...
			 			Main.Game_Screen.setFocusable(false);
			 			Main.mainFrame.remove(Main.Game_Screen);
			 			Main.Player1.credits += Main.Game_Screen.level_score;
			 			Main.Player1.score +=  Main.Game_Screen.level_score;
	...
	private void HandlekeyActions()
	{
		...
		else if(Ship_Pos_X >= 930 && current_xVel > 0)
		{
			Ship_Pos_X = 930;
	...
	public void deal_with_shots_fired(Graphics g)
	{
		//ArrayList<Shot> newshots = new ArrayList <Shot>();
		 ...
		 {  
			shots.get(i).move();			
				g.drawImage(shots.get(i).weapon,(int)shots.get(i).getXPosition(),(int)shots.get(i).getYPosition(),7,21,Main.Game_Screen);	
