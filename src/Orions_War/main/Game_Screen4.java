 ...
 *   everything used for level 4 a copy of other game screens
 *
 ***************************************************************************
 **************************************************************************/
...
public class Game_Screen4 extends JPanel implements KeyListener
{
...
//Need to figure out how to do double buffering.. 
	...
	public void play() throws FileNotFoundException
	{
		System.setProperty("sun.java2d.pmoffscreen", "false");
		  System.setProperty("org.apache.pivot.wtk.disablevolatilebuffer",
			...
			Graphics g = Main.Game_Screen4.getGraphics();
		    g.drawImage(img1, 0, 0, 1010,805, Main.Game_Screen4);
		...
		{ 		
			public void run()
			{Main.Game_Screen4.setDoubleBuffered(true);
			
				while(!levelFinished && Ship_Alive)
				{
					Graphics g = Main.Game_Screen4.getGraphics();
				
					g.drawImage(img1, 0, 0, 1000,800, Main.Game_Screen4);
					g.drawImage(Main.Player1.ship,Ship_Pos_X,Ship_Pos_Y, (Main.Player1.ship.getWidth())/2,(Main.Player1.ship.getHeight())/2,Main.Game_Screen4);
		...
		Runnable r13 = new Runnable()
		{
			...
			{   
				...
				Main.Game_Screen4.setDoubleBuffered(true);				
			    	  ...
			    	  level =  new File(System.getProperty("user.dir")+"/levels/level4.txt");  
				...
				while(!levelFinished && Ship_Alive)
				{  
					if(Main.Game_Screen4.all_spawned == 0)
					{	
				      int num = 0;
				      int [] nums = new int[6];
				      boolean done = false;
				      ...
				      {  
				    	...
				    	{  
				    	  ...
				    	  {				    			
				    		  ...
				    		  Main.Game_Screen4.xPosition = generator.nextInt(940);
				    		  Enemy_Asteroids.add(new Asteroids(nums[0],nums[1] + Main.Game_Screen4.xPosition,nums[2],nums[3],nums[4],nums[5]));
				    		  try
								{	
									Thread.sleep(1000);
								}
				      ...
				      sc.close();
					Main.Game_Screen4.all_spawned = 1;
				  }
			...
			{     
				...
				while((Ship_Alive && !levelFinished && set == 0) || (!Ship_Alive && !levelFinished && set == 0))
				{
				    if(!Ship_Alive)
				    {
				    		...
				    		Main.Game_Screen4.setVisible(false);
				 			Main.Game_Screen4.setEnabled(false);
				 			Main.Game_Screen4.setFocusable(false);
				 			Main.mainFrame.remove(Main.Game_Screen4);
				    ...
				    else if(enemies.enemies.size() == 0 && Enemy_Asteroids.size() == 0 && Main.Game_Screen4.all_spawned == 1)
				    {
				    	thr1.stop();
			    	    thr2.stop();
			    		thr3.stop();
			    		...
			    		Main.Game_Screen4.setVisible(false);
			 			...
			 			Main.Game_Screen4.setFocusable(false);
			 			Main.mainFrame.remove(Main.Game_Screen4);
			 			Main.Player1.credits += Main.Game_Screen4.level_score;
			 			Main.Player1.score +=  Main.Game_Screen4.level_score;
	...
	public void deal_with_shots_fired(Graphics g)
	{
		//ArrayList<Shot> newshots = new ArrayList <Shot>();
		 ...
		 {  
			shots.get(i).move();			
				g.drawImage(shots.get(i).weapon,(int)shots.get(i).getXPosition(),(int)shots.get(i).getYPosition(),7,21,Main.Game_Screen4);	
	...
	public void spawn_the_asteroids(Graphics g)
	{
		for(int y = 0; y < Enemy_Asteroids.size(); y++)
		{
			...
			g.drawImage(Enemy_Asteroids.get(y).sprite,(int)Enemy_Asteroids.get(y).getXPosition(),(int)Enemy_Asteroids.get(y).getYPosition(),(int)Enemy_Asteroids.get(y).sprite.getWidth(),(int)Enemy_Asteroids.get(y).sprite.getHeight(),Main.Game_Screen4);
		}
	}
